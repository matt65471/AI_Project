...
def play(checkpoint_path="dqn_pong_nature_seed42_model.pth", episodes=5):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    env = make_atari_env("ALE/Pong-v5", render_mode="rgb_array")

    # Records every episode to videos/pong/ folder
    env = RecordVideo(env, video_folder="videos/pong/", episode_trigger=lambda e: True)
